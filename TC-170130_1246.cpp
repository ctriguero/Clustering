...
//////	// Cluster Index KK CHECKS
//////	cout << "ci = " << AtomSet_object.GetNode(0).GetClusterIndex() << std::endl ;	
//////	AtomSet_object.ChangeNodeClusterIndex (0,7) ;
//////	cout << "ci = " << AtomSet_object.GetNode(0).GetClusterIndex() << std::endl ;
//////	for ( size_t i = 0; i < NodeAtoms; ++i ){ cout << "Cluster Index Node = " << AtomSet_object.GetNode(i).GetClusterIndex() << std::endl ;}
//////	for ( size_t i = 0; i < SeaAtoms; ++i ){ std::cout << "Cluster Index Sea = " << AtomSet_object.GetSea(i).GetClusterIndex() << std::endl ;}
...
//						std::cout << "Inside loop k " << k << std::endl ;
					...
					{
						rho = float(bin[k])/Vol ;
						//if ( rho >= rhoT ) { goto SkipConnection ; }
					}
					std::cout << "ATOMS " << i << " AND " << j << " ARE CONNECTED" << std::endl ;


					// Connecting atoms: Assingning the same cluster index.
					...
					// unsigned int OldClusterSize = iCluster.size() ;

					unsigned int Cj = AtomSet_object.GetNode(j).GetClusterIndex() ;

					// Connecting cluster associated to i with cluster associated to j
					for ( int m=0; m<iCluster.size(); m++) { AtomSet_object.ChangeNodeClusterIndex (iCluster[m],Cj) ; } 
...
//				std::cout << std::endl ;
	...
	// Extract cluster information now
