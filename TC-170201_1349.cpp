...
int main( int argc, const char* argv[] )
{
	...
	double hc = 10.0 ; //6.0*R2 ;		// Cylinder height (Bin cylinder) //in sea atoms diameter units.
	double rc = 1.5*R2 ;		// Cylinder radius //in sea atoms diameter units.
	double rhoT = 0.001 ;		// Connectivity transition density threshold. 0 => all disconnected 100 => all connected  sea=0.0181
	//----------------------------------------------------------------------------------------------------------------------------
	//----------------------------------------------------------------------------------------------------------------------------		
	//hc = hc*2*R2
	...
	ofstream ClusterSizes ;
	ClusterSizes.open ("ClusterSizes.dat", ios::out | ios::trunc); // Detected cluster sizes in descending order
...
//			std::cout << "hc = " << hc << std::endl ;
			...
			std::vector<int> bin(dim_nbins, 0) ; //dim_nbins + 1

			// BEGIN Loop over sea atoms
	...
	std::vector<int> UsefulClusterIndex ;

	// Recognizes all non repeated cluster indexes and stores them in UsefulClusterIndex
	for ( int i=0; i<NodeAtoms; i++)
	{
		unsigned int skip=0 ;
		for ( int j=0; j<NodeAtoms; j++)
		{
			unsigned int Cj=AtomSet_object.GetNode(j).GetClusterIndex() ;
			if ( ( i == Cj ) && ( skip == 0 ) )
			{
				UsefulClusterIndex.push_back(i) ;
				skip = 1 ;
			}
		}
	}
	unsigned int ClusterNumber =  UsefulClusterIndex.size () ;
	std::cout << YELLOW << "    -> Detected " << BOLDYELLOW << ClusterNumber << RESET << YELLOW << " clusters" << RESET << endl ;
	...
	std::vector<int> size(ClusterNumber,0) ;
	int ClusterIndex=0 ;
	for ( int j=0; j<ClusterNumber; j++)
	{
		...
		if ( j <= 9 ) { ss << "Cluster_no_000" << j+1 << ".xyz" ;}
		if ( ( j > 9 ) && ( j <= 99 ) ) { ss << "Cluster_no_00" << j+1 << ".xyz" ;}
		if ( ( j > 99 ) && ( j <= 999 ) ) { ss << "Cluster_no_0" << j+1 << ".xyz" ;}
		if ( ( j > 999 ) && ( j <= 9999 ) ) { ss << "Cluster_no_" << j+1 << ".xyz" ;}
		
		ClusterFile.open(ss.str().c_str());

		int ClusterAtoms = 0 ;
		for ( int k=0; k<NodeAtoms; k++)
		{
			Atom AtomK = AtomSet_object.GetNode(k) ;
			unsigned int Ck = AtomK.GetClusterIndex() ;

			if ( Ck == UsefulClusterIndex[j] ) 
			{ 
				// This measures the size of the cluster
				ClusterAtoms++ ; 
				// Generates an xyz file for each detected cluster
				ClusterFile << NodeType << "\t" << AtomK.GetX()<< "\t" << AtomK.GetY() << "\t" << AtomK.GetZ() << std::endl ;
		...
		size[j] = ClusterAtoms ;
//		std::cout << " size= " << ClusterAtoms << std::endl ;
	}
	...
	{
		std::cout << "\t" << BLUE   << "    * Largest cluster contains " << RESET << BOLDBLUE << size[0] << RESET << BLUE << " nodes" << RESET << std::endl ;
		std::cout << "\t" << BLUE   << "    * Smallest cluster contains " << RESET << BOLDBLUE << size[ClusterNumber-1] << RESET << BLUE << " nodes" << RESET << std::endl ;
	}

	for ( int i=0; i<size.size(); i++) { ClusterSizes << size[i] << std::endl ; }
	std::cout << YELLOW << "    -> Cluster sizes stored in descending order in " << BOLDYELLOW << " ClusterSizes.dat" << RESET << std::endl ;
...
//					// END Store clusters
	...
	// Execution time
 	std::cout << endl ;
	std::cout << "    Execution time: " << BOLDWHITE << ( stop_s - start_s )/double(CLOCKS_PER_SEC) << RESET << "seconds" << std::endl ;
	std::cout << endl ;
