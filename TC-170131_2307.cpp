...
//			std::cout << "hc = " << hc << std::endl ;
	...
	for ( int j=0; j<ClusterNumber; j++)
	{
		...
		{
			unsigned int Ck = AtomSet_object.GetNode(k).GetClusterIndex() ;
//			std::cout << "Ck=" << Ck << std::endl ;
			//std::cout << "use=" << UsefulClusterIndex[j] << std::endl ;

			if ( Ck == UsefulClusterIndex[j] ) 
			{ 
				ClusterAtoms++ ; 
//				std::cout << ClusterAtoms << std::endl ;
			}
		}
		size[j] = ClusterAtoms ;
//		std::cout << " size= " << ClusterAtoms << std::endl ;
	...
	//for ( int j=0; j<ClusterNumber; j++) {std::cout << " useful index= " << UsefulClusterIndex[j] << std::endl ; }
	...
	for ( int j=0; j<size.size(); j++) { ClusterSizes << size[j] << std::endl ; }
...
//					// END Store clusters
	...
	//Store node indexes and coordinates etc etc etc...
